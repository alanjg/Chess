...
void Model::Init(TriangleSet * t)
{
	...
	m_indexBuffer.reset(new DataBuffer(DIRECTX.Device, D3D11_BIND_INDEX_BUFFER, &t->Indices()[0], t->numIndices() * sizeof(short)));
...
void Model::Render(XMMATRIX * projView)
{
	...
	DIRECTX.Context->IASetIndexBuffer(m_indexBuffer->GetBuffer(), DXGI_FORMAT_R16_UINT, 0);
	UINT offset = 0;
...
bool Model::Pick(XMVECTOR rayOrigin, XMVECTOR rayDirection, float& tMin)
{
	...
	if (XNA::IntersectRayAxisAlignedBox(rayOrigin, rayDirection, &m_boundingBox, &tHit))
	{
		for (int i = 0; i < m_triangleSet->numIndices(); i += 3)
		{
			...
			for (int j = 0; j < 3; j++)
			{				
				Vertex& v = m_triangleSet->Vertices()[i + j];
				vFinal[j].m128_f32[0] = v.Pos.x;
				vFinal[j].m128_f32[1] = v.Pos.y;
				vFinal[j].m128_f32[2] = v.Pos.z;
